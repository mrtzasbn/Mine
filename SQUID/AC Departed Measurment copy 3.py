...
for name02, group02 in grouped02:
    zero_time = group02["Time"].iloc[0]
    group02["Time"] = group02["Time"] - zero_time
    ax.plot((group02["Time"]/60), group02['m" (emu)']*1E+6, label=f'Field {int(name02)/10} (mT), 10 min', marker='*', color="blue")

for name03, group03 in grouped03:
    zero_time = group03["Time"].iloc[0]
    group03["Time"] = group03["Time"] - zero_time
    ax.plot(group03["Time"]/60, group03['m" (emu)']*1E+6, label=f'Field {int(name03)/10} (mT), 1 Hz', marker='+', color="red")

for name04, group04 in grouped04:
    zero_time = group04["Time"].iloc[0]
    group04["Time"] = group04["Time"] - zero_time
    ax.plot(group04["Time"]/60, group04['m" (emu)']*1E+6, label=f'Field {int(name04)/10} (mT)', marker='h', color="green")
...
for name04, group04 in grouped04:
    zero_time = group04["Time"].iloc[0]
    group04["Time"] = group04["Time"] - zero_time
    ax_inset.plot(group04["Time"]/60, group04['m" (emu)']*1E+6, label=f'Field {int(name04)/10} (mT)', marker='h', color="green")
...
for name04, group04 in grouped03:
    zero_time = group03["Time"].iloc[0]
    group03["Time"] = group03["Time"] - zero_time
    ax_inset.plot(group03["Time"]/60, group03['m" (emu)']*1E+6, label=f'Field {int(name03)/10} (mT), 1 Hz', marker='+', color="red")
ax_inset.set_xlabel("Time (min)")
ax_inset.set_ylabel('m" $\\times 10^{{{-6}}}$ (emu)')
ax_inset.tick_params(axis='y')
ax_inset.grid(True)
