...
file =   r"D:\MyData\CERN\R169-5\SQUID\AC Modified\AC-5K_Field_R86-5_High_ModifiedMoreAC80mT.ac.dat"
file02 = r"D:\MyData\CERN\R169-5\SQUID\AC Modified\AC-5K_Field_R86-5_High_ModifiedMoreAC80mT_move.ac.dat"
# file03 = r"D:\MyData\CERN\R86-5\SQUID\AC Modified 02\AC-5K_Field_R86-5_High_ModifiedMoreAC80mT_AC.ac.dat"
...
for name, group in grouped:
    ...
    # max = 1
    ax.plot(group.reset_index(drop=True).index + 1, ((group.reset_index(drop=True)['m" (emu)']-min)/max)*1, label=f'Field {int(name)/10} (mT)', marker = 'o', color="black")
...
for name02, group02 in grouped02:
    ...
    # max = 1 
    ax.plot(group02.reset_index(drop=True).index + 1, ((group02.reset_index(drop=True)['m" (emu)']-min)/max)*1, label=f'Field {int(name02)/10} (mT), Move', marker = '*')
...
# plt.savefig("ACNormal" + '.pdf', format='pdf', bbox_inches='tight')
# plt.savefig("ACNormal" + '.png', format='png', bbox_inches='tight')
