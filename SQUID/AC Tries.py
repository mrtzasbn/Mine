...
file =   r"D:\MyData\IS258A\AC-77K_IS258A_High_80mT.ac.dat"
# file02 = r"D:\MyData\IS258A\AC-5K_IS258A_High_10mT.ac.dat"
...
for name, group in grouped:
    ...
    max = 1
    ax.plot(group.reset_index(drop=True).index + 1, ((group.reset_index(drop=True)['m" (emu)']-min)/max)*1E6, label=f'Field {int(name)/10} (mT), 77K.', marker = 'o', color="black")
...
#     max = 1 
#     ax.plot(group02.reset_index(drop=True).index + 1, ((group02.reset_index(drop=True)['m" (emu)']-min)/max)*1E6, label=f'Field {int(name02)/10} (mT), 5K', marker = '*')
...
#     max = 1
#     ax_inset.plot(group02.reset_index(drop=True).index + 1, ((group02.reset_index(drop=True)['m" (emu)']-min)/max)*1E6, label=f'Field {int(name02)/10} (mT), 5K', marker = '+')
...
plt.savefig("AC_77K" + '.pdf', format='pdf', bbox_inches='tight')
plt.savefig("AC_77K" + '.png', format='png', bbox_inches='tight')
